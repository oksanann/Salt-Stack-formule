...
total-commander-package-file-managed-installer:
  file.managed:
...
total-commander-package-cmd-run-installer:
  cmd.run:
    - name: '"{{ win.installer_path }}" {{ win.install_args }}'
    - require:
      - file: total-commander-package-file-managed-installer

{%- elif win_method == 'chocolatey' %}

total-commander-package-chocolatey-installed:
...
total-commander-package-winrepo-pkg-installed:
  pkg.installed:
    - name: {{ winrepo_name }}
{%- if pkg_version %}
    - version: '{{ pkg_version }}'
...
total-commander-package-pkg-installed:
  pkg.installed:
    - name: {{ cfg.pkg.name }}
...
{%- if cfg.get('repo', {}).get('name') %}
    - require:
      - pkgrepo: total-commander-repository-install-pkgrepo-managed
{%- endif %}
